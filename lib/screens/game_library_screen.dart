...
class _GameLibraryScreenState extends State<GameLibraryScreen> {
  ...
  void _handleSteamSyncState() {
    ...
    final SteamSyncPhase? previous = _lastSteamSyncPhase;
  ...
  List<GameLibraryEntry> get _visibleGames {
    ...
    if (_favoriteOnly) {
      ...
      filtered = filtered.where((game) => game.status == _statusFilter);
    }
  ...
  Future<void> _toggleFavorite(GameLibraryEntry game) async {
  ...
  Future<void> _addGame() async {
    ...
    await GameLibraryService.addManualGame(
      ...
      catalogPlatforms: catalog?.platforms ?? const <String>[],
    );
    await _load();
  }
  ...
  Future<void> _enrichGame(GameLibraryEntry game) async {
    ...
    final GameCatalogResult? result = await showDialog<GameCatalogResult>(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) =>
          _CatalogPickerDialog(initialQuery: game.title),
    );
    ...
    try {
      ...
      await _load();
      if (mounted) {
        _message('Jaquette et informations mises à jour pour ${result.title}.');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _openAccounts() async {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute<void>(builder: (_) => const GamingAccountsScreen()),
    );
    if (mounted) {
      await _load();
    }
  ...
  Future<void> _editGame(GameLibraryEntry game) async {
    ...
    final _GameEditResult? result = await showModalBottomSheet<_GameEditResult>(
      context: context,
      ...
      builder: (sheetContext) => _GameEditSheet(game: currentGame),
    );
    ...
    if (result.delete) {
      final bool confirmed = await _confirmDelete(currentGame);
      if (!confirmed) {
        return;
      }
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Rechercher dans mes ${_games.length} jeux...',
                  ...
                  SizedBox(
                    ...
                    child: ListView(
                      ...
                      children: [
                        _FilterChip(
                          label: 'TOUS',
                          selected: _statusFilter == null && !_favoriteOnly,
                          onTap: () {
                            setState(() {
                              _statusFilter = null;
                              _favoriteOnly = false;
                        ...
                        ...GameStatus.values
                            .where(
                              (status) => status != GameStatus.unclassified,
                            )
                            .map(
                              (status) => _FilterChip(
                                label: status.label.toUpperCase(),
                                selected:
                                    _statusFilter == status && !_favoriteOnly,
                                onTap: () {
                                  setState(() {
                    ...
                    children: [
                      ...
                      PopupMenuButton<_LibrarySort>(
                        ...
                        itemBuilder: (context) => _LibrarySort.values
                            .map(
                              (value) => PopupMenuItem<_LibrarySort>(
                                value: value,
                                child: Row(
                                  children: [
                                    if (value == _sort)
                    ...
                    ...games.map(
                      (game) => Padding(
                        ...
                        child: _GameCard(
                          ...
                          onFavorite: () => _toggleFavorite(game),
                          onEnrich:
        ...
        children: [
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: Text(
                steamBusy ? 'SYNCHRONISATION...' : 'SYNCHRONISER TOUT',
              ),
              style: OutlinedButton.styleFrom(
                foregroundColor: const Color(0xffb7d8ff),
                side: const BorderSide(color: Color(0xff315c83)),
              ),
            ),
          ),
          if (steamBusy &&
    ...
    return Padding(
      ...
      child: ChoiceChip(
        ...
        labelStyle: TextStyle(
          color: selected ? const Color(0xff21150e) : Colors.white70,
          fontSize: 10.5,
          fontWeight: FontWeight.bold,
        ),
        side: BorderSide(
          color: selected ? const Color(0xffffc857) : Colors.white10,
        ),
      ),
    );
  }
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _GameCover(game: game, width: 70, height: 96),
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        if (game.status != GameStatus.unclassified)
                          Container(
                            padding: const EdgeInsets.symmetric(
                              horizontal: 7,
                              vertical: 3,
                    ...
                    if (game.releaseYear != null || game.genres.isNotEmpty) ...[
                      const SizedBox(height: 4),
                      Text(
                        [
                        ...
                        children: [
                          Expanded(
                            child: LinearProgressIndicator(
                              value: game.bestCompletionPercent! / 100,
                              minHeight: 4,
...
class _AddGameDialogState extends State<_AddGameDialog> {
  ...
  void _finish({required bool manual}) {
    ...
    final GameCatalogResult? selected = manual ? null : _selected;
              ...
              child: _results.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        final GameCatalogResult result = _results[index];
                        final bool selected = _selected?.id == result.id;
                      ...
                      Expanded(
                        child: DropdownButtonFormField<GamePlatform>(
                          ...
                          items: GamePlatform.values
                              .map(
                                (platform) => DropdownMenuItem<GamePlatform>(
                                  value: platform,
...
class _CatalogPickerDialog extends StatefulWidget {
  ...
  const _CatalogPickerDialog({required this.initialQuery});
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(
              searching ? Icons.travel_explore : Icons.image_search_outlined,
...
class _CatalogResultTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      color: selected ? const Color(0xff302716) : const Color(0xff202326),
      borderRadius: BorderRadius.circular(13),
      child: InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(13),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      [
                        if (result.releaseYear != null) '${result.releaseYear}',
...
class _GameEditResult {
  ...
  const _GameEditResult.delete() : game = null, delete = true;
}
...
class _GameEditSheet extends StatefulWidget {
  ...
  @override
  State<_GameEditSheet> createState() => _GameEditSheetState();
}
...
class _GameEditSheetState extends State<_GameEditSheet> {
  ...
  _AchievementFilter _achievementFilter = _AchievementFilter.all;
  final Set<GamePlatform> _manualEditorsExpanded = <GamePlatform>{};
  ...
  void initState() {
    ...
    _profiles = List<GamePlatformProfile>.from(game.resolvedPlatformProfiles);

    final GamePlatformProfile? steam = game.platformProfile(GamePlatform.steam);

    _selectedPlatform =
        steam?.platform ??
        (_profiles.isNotEmpty ? _profiles.first.platform : game.platform);
  }
  ...
  List<int> get _visibleAchievementIndexes {
    final GamePlatformProfile? profile = _selectedProfile;

    if (profile == null) {
      return const <int>[];
    }

    final List<GameAchievementDetail> details = profile.achievementDetails;

    return List<int>.generate(details.length, (index) => index).where((index) {
      final bool unlocked = details[index].isUnlocked;
  ...
  void _replaceProfile(GamePlatformProfile profile) {
  ...
  void _toggleAchievement(int index, bool unlocked) {
    final GamePlatformProfile? profile = _selectedProfile;
    ...
    final List<GameAchievementDetail> details =
        List<GameAchievementDetail>.from(profile.achievementDetails);

    final GameAchievementDetail achievement = details[index];

    if (achievement.platformUnlocked) {
      return;
    }

    details[index] = achievement.withManualState(unlocked);

    final GamePlatformProfile next = profile.copyWith(
      achievementDetails: details,
      achievementCatalogInitialized:
          profile.achievementCatalogInitialized || details.isNotEmpty,
    );

    _replaceProfile(
      next.copyWith(achievements: next.computedAchievementSummary),
    );
  }

  void _save() {
    final bool titleLocked = widget.game.hasOfficialPlatformConnection;
    final String title = titleLocked ? widget.game.title : _title.text.trim();

    if (title.isEmpty) {
      return;
    }

    final List<GamePlatformProfile> normalizedProfiles = _profiles
        .map(
          (profile) => profile.copyWith(
            achievements: profile.computedAchievementSummary,
          ),
        )
        .toList();
    ...
    Navigator.pop(context, _GameEditResult.save(updated));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final GameLibraryEntry game = widget.game;
    final GamePlatformProfile? best = _bestProfile(_profiles);
    final GamePlatformProfile? selected = _selectedProfile;
    final List<int> visibleAchievementIndexes = _visibleAchievementIndexes;
    ...
    final int selectedLockedCount =
        (selected?.achievementDetails.length ?? 0) - selectedUnlockedCount;
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _GameCover(game: game, width: 72, height: 102),
              ...
              TextField(
                controller: _title,
                readOnly: game.hasOfficialPlatformConnection,
                enableInteractiveSelection: true,
                style: const TextStyle(color: Colors.white),
                decoration: InputDecoration(
                  labelText: 'Nom du jeu',
                  suffixIcon: game.hasOfficialPlatformConnection
                      ? const Icon(
                          Icons.lock_outline,
                          color: Colors.white38,
                          size: 19,
                      ...
                      : null,
                  helperText: game.hasOfficialPlatformConnection
              ...
              DropdownButtonFormField<GameStatus>(
                initialValue: _status == GameStatus.unclassified
                    ? null
                    : _status,
                dropdownColor: const Color(0xff23262a),
                decoration: const InputDecoration(
                ...
                items: GameStatus.values
                    .where((status) => status != GameStatus.unclassified)
                    .map(
                      (status) => DropdownMenuItem<GameStatus>(
                        value: status,
                        child: Text(status.label),
              ...
              const SizedBox(height: 12),
              if (best != null && best.completionPercent != null)
                _BestCompletionCard(profile: best)
              else
              ...
              for (final GamePlatformProfile profile in _profiles) ...[
                _PlatformProgressCard(
                  profile: profile,
                  selected: profile.platform == _selectedPlatform,
                  onTap: () {
                    setState(() {
                      _selectedPlatform = profile.platform;
                      _achievementFilter = _AchievementFilter.all;
                    });
                  },
                ),
                const SizedBox(height: 8),
              ...
              if (selected != null) ...[
                ...
                Row(
                  children: [
                    Expanded(
                      child: Text(
                        selected.achievementCatalogKnownEmpty
                            ? selected.platform.label.toUpperCase()
                            : selected.platform == GamePlatform.playstation
                    ...
                    if (!selected.achievementCatalogKnownEmpty &&
                ...
                if (selected.achievementCatalogKnownEmpty)
                ...
                else if (selected.achievementDetails.isNotEmpty) ...[
                  Wrap(
                    spacing: 7,
                    runSpacing: 7,
                    children: _AchievementFilter.values.map((filter) {
                      final int count = switch (filter) {
                        _AchievementFilter.all =>
                          selected.achievementDetails.length,
                        _AchievementFilter.unlocked => selectedUnlockedCount,
                        _AchievementFilter.locked => selectedLockedCount,
                      };

                      return ChoiceChip(
                        label: Text('${filter.label} ($count)'),
                        selected: _achievementFilter == filter,
                        onSelected: (_) {
                          setState(() {
                            _achievementFilter = filter;
                          });
                        },
                        selectedColor: const Color(0xff3a2e19),
                        backgroundColor: const Color(0xff15181b),
                        side: BorderSide(
                          color: _achievementFilter == filter
                              ? const Color(0xffffc857)
                              : Colors.white12,
                        ),
                        labelStyle: TextStyle(
                  ...
                  const SizedBox(height: 10),
                  if (visibleAchievementIndexes.isEmpty)
                    Container(
                      width: double.infinity,
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                      ...
                      child: Text(
                        _achievementFilter == _AchievementFilter.unlocked
                    ...
                    ...visibleAchievementIndexes.map((index) {
                      final GameAchievementDetail achievement =
                          selected.achievementDetails[index];

                      final String sourceLabel = achievement.platformUnlocked
                          ? 'Confirmé par ${selected.platform.label}'
                          : achievement.manuallyUnlocked
                          ? 'Coché manuellement • en attente de confirmation'
                          : 'Non obtenu';

                      final Widget icon =
                          achievement.iconUrl == null ||
                              achievement.iconUrl!.isEmpty
                          ? Icon(
                      ...
                      return Container(
                        ...
                        child: CheckboxListTile(
                          ...
                          checkColor: const Color(0xff21150e),
                          title: Text(
                            achievement.name,
                            style: const TextStyle(
                              color: Colors.white,
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              Text(
                                sourceLabel,
                                style: TextStyle(
                                  color: achievement.platformUnlocked
                                      ? const Color(0xff8fd5a6)
                                      : achievement.manuallyUnlocked
                                      ? const Color(0xff8ebce9)
                                      : Colors.white24,
                                  fontSize: 9.5,
                                ),
                              ),
                          ...
                          onChanged: achievement.platformUnlocked
                              ? null
                              : (value) {
                                  setState(() {
                                    _toggleAchievement(index, value == true);
                                  });
                                },
                        ),
                      );
                    }),
                ] else if (selected.hasAchievementData)
                  _ManualSummaryEditor(
                    key: ValueKey<String>('manual_${selected.platform.name}'),
                    profile: selected,
                    onChanged: (summary) {
                      setState(() {
                        _replaceProfile(
                          selected.copyWith(achievements: summary),
                        );
                      });
                    },
                  )
                else if (_manualEditorsExpanded.contains(selected.platform))
                  _ManualSummaryEditor(
                    key: ValueKey<String>('manual_${selected.platform.name}'),
                    profile: selected,
                    onChanged: (summary) {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        OutlinedButton.icon(
                          onPressed: () {
                            setState(() {
                              _manualEditorsExpanded.add(selected.platform);
                            });
                          },
                          icon: const Icon(Icons.edit_note, size: 18),
                          label: const Text('RENSEIGNER MANUELLEMENT'),
                          style: OutlinedButton.styleFrom(
                            foregroundColor: const Color(0xffcbb3e8),
                            side: const BorderSide(color: Color(0x55CBB3E8)),
              ...
              Row(
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () {
                        Navigator.pop(context, const _GameEditResult.delete());
                      },
  ...
  GamePlatformProfile? _bestProfile(List<GamePlatformProfile> profiles) {
    GamePlatformProfile? best;
    int bestValue = -1;

    for (final GamePlatformProfile profile in profiles) {
...
class _BestCompletionCard extends StatelessWidget {
  ...
  const _BestCompletionCard({required this.profile});

  @override
  Widget build(BuildContext context) {
    final int value = profile.completionPercent ?? 0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            value == 100 ? '🏆 100 % COMPLÉTÉ' : 'MEILLEURE COMPLÉTION',
            style: const TextStyle(
              color: Color(0xffffc857),
              fontWeight: FontWeight.bold,
              fontSize: 12,
...
class _UnknownCompletionCard extends StatelessWidget {
  const _UnknownCompletionCard();
...
class _PlatformProgressCard extends StatelessWidget {
  final GamePlatformProfile profile;
  final bool selected;
  final VoidCallback onTap;
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      color: selected ? const Color(0xff24202d) : const Color(0xff15181b),
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(12),
            border: Border.all(
              color: selected ? const Color(0xffb69bdc) : Colors.white10,
            ),
          ),
          child: Column(
            children: [
              Row(
                children: [
                  Icon(
                    _platformIcon(profile.platform),
                    color: selected ? const Color(0xffcbb3e8) : Colors.white54,
                    size: 18,
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                  ...
                  Expanded(
                    child: Text(
                      profile.achievementCatalogKnownEmpty
                          ? profile.platform == GamePlatform.playstation
  ...
  static IconData _platformIcon(GamePlatform platform) {
    switch (platform) {
...
class _ManualSummaryEditor extends StatefulWidget {
  ...
  @override
  State<_ManualSummaryEditor> createState() => _ManualSummaryEditorState();
}

class _ManualSummaryEditorState extends State<_ManualSummaryEditor> {
  ...
  void initState() {
    ...
    final GameAchievementSummary a = widget.profile.computedAchievementSummary;

    _unlocked = TextEditingController(text: '${a.unlocked}');
    _total = TextEditingController(text: '${a.total}');
    _bronzeUnlocked = TextEditingController(text: '${a.bronzeUnlocked}');
    _bronzeTotal = TextEditingController(text: '${a.bronzeTotal}');
    _silverUnlocked = TextEditingController(text: '${a.silverUnlocked}');
    _silverTotal = TextEditingController(text: '${a.silverTotal}');
    _goldUnlocked = TextEditingController(text: '${a.goldUnlocked}');
    _goldTotal = TextEditingController(text: '${a.goldTotal}');
    _platinumUnlocked = TextEditingController(text: '${a.platinumUnlocked}');
    _platinumTotal = TextEditingController(text: '${a.platinumTotal}');
    _scoreEarned = TextEditingController(text: '${a.scoreEarned}');
    _scoreTotal = TextEditingController(text: '${a.scoreTotal}');
  }
  ...
  @override
  Widget build(BuildContext context) {
    final GamePlatform platform = widget.profile.platform;
        ...
        children: [
          ...
          Expanded(
            child: TextField(
              ...
              onChanged: (_) => onChanged(),
              style: const TextStyle(color: Colors.white),
              decoration: InputDecoration(labelText: totalHint, isDense: true),
            ),
          ),
        ],
      ),
